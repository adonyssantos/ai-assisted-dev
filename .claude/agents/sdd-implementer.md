...
1. Read `spec.md`, `plan.md`, the existing tests, the board cards for this feature (`#spec/NNN-slug`), the root `AGENTS.md` and `projects/<name>/AGENTS.md`.
...
5. Move each card to `## Done` on `docs/board.md` only when its tests pass (or update Jira/GitHub per `docs/framework/task-tracking.md`). If the spec/plan is wrong or incomplete, stop and report — never improvise beyond the spec.
