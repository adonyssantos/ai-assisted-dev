...
3. Decompose it into **candidate features**. For each: a one-line WHAT and WHY, **no technology**,
   and a proposed `NNN-slug` id (continue the numbering already used in `specs/`).
...
Return: the candidate-feature list, the open questions, and the build order. Recommend the main
agent run `/specify` for the first feature (or launch `sdd-specifier`).
